# Module 4 — Analysis formatting (JSON cards + page render)
...
def test_card_schema_has_query_and_answer_keys(make_card_files):
    # Expect at least {"query": ..., "answer": ...}; cover scalar and table
...
def test_table_answers_are_list_of_mappings_with_consistent_keys(make_card_files):
    # Table answers must be non-empty list[dict] with consistent keys
...
def test_percent_strings_have_two_decimals_strict(make_card_files):
    # Percent strings: exactly two decimals and trailing '%'; check scalar and table
...
def test_analysis_page_has_answer_labels_and_some_content(client, make_card_files):
    # GET /analysis should render; expect basic labels/selectors and content
...
def test_numeric_answer_is_formatted_to_two_decimals(client, make_card_files):
    # If renderer formats numeric to percentage with two decimals, it should appear
