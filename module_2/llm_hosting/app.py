...
def _load_llm() -> Llama:
    ...
    if _LLM is not None:
        return _LLM
    print("[llm_hosting] Initializing and loading LLM model...", file=sys.stderr, flush=True)
...
def _call_llm(program_text: str) -> Dict[str, str]:
    ...
    print("[llm_hosting] LLM called with input:", repr(program_text), file=sys.stderr, flush=True)
...
def _cli_process_file(
    ...
    print("[llm_hosting] _cli_process_file called with", in_path, file=sys.stderr, flush=True)
    ...
    try:
        for row in rows:
            program_text = (row or {}).get("program") or ""
            print("[llm_hosting] about to call LLM with:", repr(program_text), file=sys.stderr, flush=True)
            result = _call_llm(program_text)
            row["llm-generated-program"] = result["standardized_program"]
            row["llm-generated-university"] = result["standardized_university"]
