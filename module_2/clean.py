...
class LLMClient:
    """
    HTTP-only client for the local llm_hosting service.
    """

    def __init__(self, *, base_url: Optional[str] = None, timeout_s: float = 120.0) -> None:
        self.base_url = (base_url or os.getenv("LLM_HOSTING_URL") or "").strip().rstrip("/")
        if not self.base_url:
            raise ValueError("LLM_HOSTING_URL not set and no base_url provided (HTTP host required).")
        self.timeout_s = float(timeout_s)

    @staticmethod
    def _pair(d: Dict[str, Optional[str]]) -> Dict[str, Optional[str]]:
    ...
    def canonize_batch(self, program_texts: List[str]) -> List[Dict[str, Optional[str]]]:
        url = f"{self.base_url}/standardize"
        make_pair = self._pair
        out: List[Dict[str, Optional[str]]] = []

        rows_payload = [{"program": t} for t in program_texts]

        CHUNK = 20          # smaller, more reliable
        RETRIES = 2         # quick retries per chunk
        PAUSE_S = 1.0       # short backoff

        for i in range(0, len(rows_payload), CHUNK):
            chunk = rows_payload[i:i+CHUNK]
            payload = {"rows": chunk}
            ...
            for attempt in range(RETRIES + 1):
                try:
                    ...
                    out.extend([make_pair(row) for row in rows])
                    success = True
                    break
                    ...
                    if attempt < RETRIES:
                        ...
                        print(f"[LLM] chunk {i}-{i+len(chunk)-1} failed: {type(e).__name__}")
                        out.extend([{"program_canon": None, "university_canon": None}] * len(chunk))

        # pad/trim defensively
        n = len(program_texts)
        if len(out) < n:
            out += [{"program_canon": None, "university_canon": None}] * (n - len(out))
        return out[:n]
...
def extend_with_llm(
    ...
    *,
    ...
    client = LLMClient(base_url=os.getenv("LLM_HOSTING_URL"), timeout_s=timeout_s)
