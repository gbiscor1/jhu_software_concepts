...
def test_init_db_main_applies_schema_and_seeds(monkeypatch, tmp_path, fake_conn_factory): # pylint: disable=too-many-locals
    """
    ...
    """
    init_db = pytest.importorskip(
        "src.scripts.init_db",
        reason="init_db script module not present",
    )
...
def test_init_db_seed_handles_missing_files(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, fake_conn_factory
):
    """
    ...
    """
    init_db = pytest.importorskip(
        "src.scripts.init_db", reason="init_db script module not present"
    )
    ...
    monkeypatch.setattr(init_db, "get_conn", lambda: conn, raising=False)
    monkeypatch.setattr(
        init_db, "load_applicants",
        lambda rows: {"attempted": 0, "inserted": 0, "skipped": 0},
        raising=False,
    )

    # capture printed output without using broad exceptions
    printed: dict[str, object] = {}
    def _capture_print(obj, *_args, **_kwargs):
        printed.setdefault("obj", obj)
    monkeypatch.setattr(builtins, "print", _capture_print, raising=False)
    ...
    if hasattr(init_db, "seed_from_json") and callable(init_db.seed_from_json):
        ...
        fn = _find_init_fn(init_db)  # helper you already use elsewhere
        if fn is None:
            pytest.skip("No known init entrypoint found in init_db")
        fn()

    # printed message should exist; avoid asserting exact text
